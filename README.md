...
Four condensed examples of my independent quantitative research: prediction markets, CME order flow, dealer gamma and 0DTE options, and relative-value futures.

The pattern in each is the same. Take an idea from raw data, pre-register the test where it matters, evaluate out of sample, then check whether it survives realistic execution. Negative results are kept as written. Three of the four below end in a rejection.
...
| [dealer-gamma](dealer-gamma/) | Does prior-close dealer gamma set the day's character, and can it be traded with 0DTE SPX options? | Range, yes: short-gamma days realize nearly twice the range of long-gamma days over 2,799 days, and it holds across SPX, NDX and RUT. Direction, no. Three pre-registered 0DTE trades with matched controls and honest fills all failed. |
